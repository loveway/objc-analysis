...
objc_object::clearDeallocating()
{
    if (slowpath(!isa.nonpointer)) {
        ...
        // 这个方法里面的 weak_clear_no_lock 将指向该对象的弱引用属性置为 nil
...
{
    // 如果是 TaggedPointer 则返回，不用释放，因为 TaggedPointer 的值就存在指针中
    if (isTaggedPointer()) return;  // fixme necessary?
    /* 下面判断的意思是：
       1、如果是优化过的 isa ，也就是 64 位以后用位域存储更多信息的 isa
       2、是否设置过弱引用指向
       3、是否设置过关联对象
       4、是否有 c++ 的析构函数
       5、是否有 sideTable 这张表
     如果说是 64 位以后，并且 2 、3、4、5 都没有，则直接释放，这也是这几个属性不设置会释放更快的原因
    **/
