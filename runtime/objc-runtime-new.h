...
struct method_t {
    /**
     ...
     2. 可以通过 `sel_getName()` 和 `NSStringFromSelector()` 获得
     3. 不同类中相同名字的方法，selctor 是一样的
     */
    SEL name;// 函数名
    const char *types;// 编码（返回值、参数类型）
    MethodListIMP imp;// 指向函数的的指针（函数地址）
...
// ro： readonly  t: table 只读的列表，不允许修，只包含类的初始类容
struct class_ro_t {
    uint32_t flags;
...
struct class_rw_ext_t {
    // 注意这是个常量，一开始就已经定型了
    const class_ro_t *ro;
    // 二维数组，存储着类初始的方法，里面可能存着分类，类的方法列表
