...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    /** `+load` 方法在程序运行期间只会调用一次，在 runtime 加载类、分类时候调用，顺序如下
     1. 先调用类的 `+load` 方法，按照编译顺序调用
     2. 调用子类的 `+load` 方法之前先调用父类的 `+load` 方法，然后再调用子类的 `+load`
     3. 按照编译顺序调用分类的 `+load`
     */
    call_load_methods();
    ...
    /**
     ...
     1. 调用方式上：`+load` 是通过函数地址直接调用的，`+initialize` 是通过 `objc_msgSend` 调用的
     2. 调用时刻：`+load` 是程序运行时在 runtime 加载类、分类时候调用(只会调用一次)，`+initialize` 是类在第一次收到消息时调用，每一个类只会 initialize 一次，不过父类的 `+initialize` 可能会调用多次(因为如果子类调用 `+initialize` 但是子类没有实现时，就会调用父类的)
     
     `+load` 和 `+initialize` 的调用顺序
     1. `+load`
       * 先调用类的 `+load`（按照类的编译顺序，先编译先调用），调用子类的 `+load` 方法之前会先调用父类的 `+load`
       * 再调用分类的 `+load`，按照编译顺序调用
     2. `+initialize`
       * 先初始化父类
       * 再初始化子类(最终可能是调用父类的 `+initialize`)
     */
}
...
static void schedule_class_load(Class cls)
{
    ...
    // 这里使用了递归调用，通过 cls->superclass 找到父类传进来，然后添加到 loadable_classes 数组，递归最后返回时候子类才一步步加在数组，加载在数组的最后面，而 loadable_classes 类是顺序加载的，所以这也是为什么父类的 `+load` 永远比子类的 `+laod` 先调用的原因，就像这段代码的注释 `Ensure superclass-first ordering` 一样
    schedule_class_load(cls->superclass);
    
    // 将 cls 添加到 loadable_classes 数组中，加在数组的最后面
